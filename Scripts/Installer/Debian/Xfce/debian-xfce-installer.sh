...
folder=debian-xfce-fs
if [ -d "$folder" ]; then
	first=1
	echo "skipping downloading"
fi
tarball="debian-xfce-rootfs.tar.xz"
rm -f $tarball
if [ "$first" != 1 ];then
	if [ ! -f $tarball ]; then
		echo "Download Rootfs, this may take a while base on your internet speed."
		wget --show-progress "https://github.com/NextAppsLab/LinuxBoxResources/releases/download/Debian-Xfce/debian-xfce-rootfs.tar.xz" -O $tarball
	fi
	cur=`pwd`
	mkdir -p "$folder"
	cd "$folder"
...
mkdir -p debian-xfce-binds
bin=start-debian-xfce.sh
...
command+=" -0"
command+=" -r $folder"
if [ -n "\$(ls -A debian-xfce-binds)" ]; then
    for f in debian-xfce-binds/* ;do
      . \$f
    done
fi
command+=" -b /dev"
command+=" -b /proc"
command+=" -b debian-xfce-fs/root:/dev/shm"
...
echo "export PULSE_SERVER=127.0.0.1" >> debian-xfce-fs/etc/profile

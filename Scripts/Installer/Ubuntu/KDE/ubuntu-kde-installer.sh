...
if [ "$first" != 1 ];then
	if [ ! -f $tarball ]; then
		...
		wget --show-progress "https://github.com/NextAppsLab/LinuxBoxResources/releases/download/Ubuntu-KDE/ubuntu-kde-rootfs.tar.xz" -O $tarball
	fi
	cur=`pwd`
	mkdir -p "$folder"
	cd "$folder"
...
command+=" -0"
command+=" -r $folder"
if [ -n "\$(ls -A ubuntu-kde-binds)" ]; then
    for f in ubuntu-kde-binds/* ;do
      . \$f
    done
fi
command+=" -b /dev"
